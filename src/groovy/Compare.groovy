...
class Compare {
    ...
    void compare(oldJson, newJson) {
        newJson.each { plugin ->
            ...
            if (!match) {
                tweet "$plugin.name $plugin.latest_version released: http://sheehan.github.io/grails3-plugins/#$plugin.name"
                return
            }
            ...
            if (plugin.updated != match.updated) {
                ...
                if (versions) {
                    versions.reverse().each { String version ->
                        tweet "$plugin.name $version released: http://sheehan.github.io/grails3-plugins/#$plugin.name"
                    }
                }
            }
        }
